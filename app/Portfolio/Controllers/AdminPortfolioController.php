...
use Portfolio\Contracts\PortfolioEditableService;
use Portfolio\Contracts\CanHaveDraft;
use Portfolio\Contracts\PortfolioItemEditable;
...
class AdminPortfolioController extends Controller
{
    public function ItemsList(Request $request) {
    	return view('admin.portfolio.items')->with([
		    'selectedMenuItem' => 'portfolio-items-list',
	    ]);
    }
    ...
    public function PostsListAjax(Request $request) {
    	return response()->json($this->blog->GetList()->map(function($post) {
		    /**
		     * @var PortfolioItemEditable $post
		     */
	...
	public function EditItem($itemId) {
		...
		return view('admin.portfolio.edit-item')->with([
			'_id' => $itemId,
			'selectedMenuItem' => ('new' == $itemId ? 'new-portfolio-item' : 'portfolio-items-list'),
		]);
	}

    public function ItemGetAjax($id) {
    	$item = $this->portfolio->GetItem($id);
    	$draft = [];
    	if($item instanceof CanHaveDraft && $item->canHaveDraft()) {
    		$draft = $item->getDraft()->getAll();
	    }
	    return response()->json([
	    	'item' => $item->prepareJson(),
		    'draft' => $draft,
	    ]);
    }
    ...
    public function CheckSlugAjax(Request $request) {
	    ...
	    $response = $this->portfolio->CheckSlug($request->slug);
    ...
    public function __construct(PortfolioEditableService $portfolio) {
    	$this->portfolio = $portfolio;
    }

	/**
	 * @var PortfolioEditableService
	 */
	private $portfolio;
	...
	private function _normalizePostMainInfoRequest(array $input) {
		...
		$req->slug = $input['slug'] ?? '';
		$req->status = $input['status'] ?? PortfolioPostEditable::STATUS_PRIVATE;
